...
def create_eval_dataset(
    ...
    if tokenize_fn_factory is None:
        ...
        def tokenize_fn_factory(tokenizer, max_seq_length):
            def tokenize_fn(inp):
                # Create a batch of formatted texts
                batch_texts = []
                for i in range(len(inp[text_column_names[0]])):
                    premise = inp[text_column_names[0]][i]
                    hypothesis = inp[text_column_names[1]][i] if text_column_names[1] in inp else None
                    
                    if hypothesis is None:
                        formatted_text = f"premise: {premise}"
                        assert False
                    else:
                        formatted_text = f"Premise: {premise}. Hypothesis: {hypothesis}.\nQuestion: Does the premise entail, contradict, or have a neutral relationship with the hypothesis?\nAnswer:"
