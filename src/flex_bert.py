...
all = ["create_flex_bert_mlm", "create_flex_bert_classification", "create_flex_gpt_classification"]
...
def create_flex_gpt_to_bert(
    ...
    if pretrained_checkpoint is not None:
        model = bert_layers_module.MNTPFlexBertForMaskedLM.from_composer(
            pretrained_checkpoint=pretrained_checkpoint, config=config
        )
    else:
        model = bert_layers_module.MNTPFlexBertForMaskedLM(config)
...
def create_flex_bert_to_gpt(
    ...
    if pretrained_checkpoint is not None:
        model = bert_layers_module.FlexGPTForCausalLM.from_composer(
            pretrained_checkpoint=pretrained_checkpoint, config=config
        )
    else:
        model = bert_layers_module.FlexGPTForCausalLM(config)
...
def create_flex_gpt_classification(
    num_labels: int,
    pretrained_model_name: str = "bert-base-uncased",
    model_config: Optional[dict] = None,
    tokenizer_name: Optional[str] = None,
    ...
    """FlexGPT classification model based on |:hugging_face:| Transformers.

    For more information, see `Transformers. <https://huggingface.co/transformers/>`_.

    This function creates a FlexGPT, which includes several throughput
    optimizations not available in |:hugging_face:| BERT as well as
    architecture changes based on ALiBi and Gated Linear Units.
    ...
    model_cls = bert_layers_module.FlexGPTForSequenceClassification

    if multiple_choice:
        model_cls = bert_layers_module.FlexGPTForMultipleChoice
